...
const CryptoDetails = () => {
  ...
  // external_constant to manage stats with title, value and icon
  const stats = [
    { title: 'Price to USD', value: `$ ${cryptoDetails?.price && millify(cryptoDetails?.price)}`, icon: <DollarCircleOutlined /> },
    { title: 'Rank', value: cryptoDetails?.rank, icon: <NumberOutlined /> },
    { title: '24h Volume', value: cryptoDetails?.volume===undefined?"Info ain't available":(`$ ${cryptoDetails?.volume && millify(cryptoDetails?.volume)}`), icon: <ThunderboltOutlined /> },
    { title: 'Market Cap', value: `$ ${cryptoDetails?.marketCap && millify(cryptoDetails?.marketCap)}`, icon: <DollarCircleOutlined /> },
    { title: 'All-time-high(daily avg.)', value: `$ ${cryptoDetails?.allTimeHigh?.price && millify(cryptoDetails?.allTimeHigh?.price)}`, icon: <TrophyOutlined /> },
  ];
  
  // external_constant to manage genricStats with title, value and icon
  const genericStats = [
      ...
      <Col className='stats-container'>
        ...
        className='coin-value-statistics'>
          ...
          {stats.map(({icon, title, value})=>(
            <Col key={icon+title} className='coin-stats'>
        ...
        <Col className="other-stats-info">
          <Col className="coin-value-statistics-heading">
            ...
            level={3}>Other Stats Info</Title>
             <p>An overview showing the statistics of {cryptoDetails.name}, such as the base and quote currency, the rank, and trading volume.</p>
          </Col>
          {genericStats.map(({icon, title, value})=>(
            <Col className='coin-stats' key={`coin-${title}${value}`}>
              <Col className='coin-stats-name'>
              <Text>{icon}</Text>
              <Text>{title}</Text>
              </Col>
